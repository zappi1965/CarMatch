...
export function VehicleCard({
  ...
  const toggle = () => {
    ...
    if (!showBack) {
      ...
      if (!insights)
        void api
          .get<ListingInsights>(`/vehicles/${listing.id}`)
          .then(setInsights)
          .catch(() => {})
    }
  ...
  const fuel = listing.fuelType
    ? t(`filters.fuelValues.${listing.fuelType}`, listing.fuelType)
    : null
      ...
      <Animated.View style={[styles.card, frontStyle]}>
        <View style={styles.imageWrap}>
          ...
          <View style={styles.imageBadges}>
            {dreamCandidate ? (
              <Badge label={`◆ ${t('mockup.dreamCandidate').toUpperCase()}`} tone="gold" />
            ) : null}
        ...
        <View style={styles.front}>
          ...
          {listing.monthlyCost ? (
            <View style={styles.monthlyCostBox}>
              <Text style={[typography.label, { color: colors.textMuted }]}>ECHTE KOSTEN</Text>
              <Text style={[typography.title, { color: colors.gold }]}>
                ca. {listing.monthlyCost.total} €/Monat
              </Text>
              <Text
                style={[
                  typography.badge,
                  {
                    color:
                      listing.monthlyCost.budgetFit === 'IN_BUDGET'
                        ? colors.like
                        : listing.monthlyCost.budgetFit === 'SLIGHTLY_OVER'
                          ? colors.warn
                          : colors.textFaint,
                  },
                ]}
              >
                {listing.monthlyCost.budgetFit === 'IN_BUDGET'
                  ...
                  : listing.monthlyCost.budgetFit === 'SLIGHTLY_OVER'
                    ? 'leicht drüber'
                    : listing.monthlyCost.budgetFit === 'WELL_OVER'
                      ? 'deutlich drüber'
                      : 'Budget offen'}
              </Text>
            </View>
          ...
          <View style={styles.badgeRow}>
            {listing.powerHp != null ? (
              <Chip label={`${listing.powerHp} ${t('common.hp')}`} />
            ) : null}
          ...
          {explanationText ? (
            <Text
              style={[typography.badge, { color: colors.textFaint, marginTop: spacing(2) }]}
              numberOfLines={2}
            >
              {explanationText}
            </Text>
      ...
      <Animated.View style={[styles.card, styles.back, backStyle]}>
        <ScrollView contentContainerStyle={{ paddingBottom: spacing(4) }}>
          <View style={styles.backImageWrap}>
            <Image
              source={{ uri: listing.images[1] ?? listing.images[0] }}
              style={styles.image}
              resizeMode="cover"
            />
          ...
          <View style={{ padding: spacing(4) }}>
            ...
            <View style={styles.specGrid}>
              <View style={styles.specColumn}>
                ...
                <Spec
                  label={t('card.firstRegistration')}
                  value={listing.firstRegistration ?? t('card.unknown')}
                />
                ...
                <Spec
                  ...
                  value={
                    listing.powerHp != null
                      ? `${listing.powerHp} ${t('common.hp')} / ${listing.powerKw ?? '–'} kW`
                      : t('card.unknown')
                  }
                />
                <Spec
                  label={t('card.zeroToHundred')}
                  value={
                    listing.specs?.zeroToHundred != null
                      ? `${listing.specs.zeroToHundred} s`
                      : t('card.unknown')
                  }
                />
                <Spec
                  label={t('card.topSpeed')}
                  value={
                    listing.specs?.topSpeed != null
                      ? `${listing.specs.topSpeed} km/h`
                      : t('card.unknown')
                  }
                />
                ...
                <Spec
                  ...
                  value={
                    listing.consumptionL100 != null && listing.consumptionL100 > 0
                      ? `${listing.consumptionL100} l/100 km`
                      : t('card.unknown')
                  }
                />
                <Spec
                  label={t('card.co2')}
                  value={listing.co2GKm != null ? `${listing.co2GKm} g/km` : t('card.unknown')}
                />
              </View>
              <View style={[styles.specColumn, styles.specColumnRight]}>
                <Spec label={t('card.transmission')} value={transmission ?? t('card.unknown')} />
                <Spec
                  label={t('card.drivetrain')}
                  value={listing.drivetrain ?? t('card.unknown')}
                />
                <Spec
                  label={t('card.bodyType')}
                  value={
                    listing.bodyType
                      ? t(`filters.bodyValues.${listing.bodyType}`, listing.bodyType)
                      : t('card.unknown')
                  }
                />
                ...
                <Spec
                  label={t('card.seller')}
                  value={listing.sellerType === 'DEALER' ? t('card.dealer') : t('card.private')}
                />
              ...
              <View style={{ flexDirection: 'row', gap: spacing(2), marginTop: spacing(4) }}>
                {scoreOf('performance') ? (
                  <ScoreDial
                    ...
                    value={scoreOf('performance')!.value}
                    confidence={scoreOf('performance')!.confidence}
                    color={colors.like}
                ...
                {scoreOf('everyday') ? (
                  <ScoreDial
                    label={t('scores.everyday')}
                    value={scoreOf('everyday')!.value}
                    confidence={scoreOf('everyday')!.confidence}
                    color={colors.info}
                    estimatedLabel={t('card.estimated')}
                  />
                ) : null}
                {scoreOf('priceValue') ? (
                  <ScoreDial
                    label={t('scores.priceValue')}
                    value={scoreOf('priceValue')!.value}
                    confidence={scoreOf('priceValue')!.confidence}
                    color={colors.gold}
                    estimatedLabel={t('card.estimated')}
                  />
                ) : null}
            ...
            {insights?.monthlyCost ? (
              <View style={styles.costBreakdown}>
                <Text style={[typography.label, { color: colors.gold, marginBottom: spacing(2) }]}>
                  Monatskosten
                </Text>
                <Spec label="Wertverlust" value={`${insights.monthlyCost.depreciation} €/Monat`} />
                <Spec label="Versicherung" value={`${insights.monthlyCost.insurance} €/Monat`} />
                <Spec label="Steuer" value={`${insights.monthlyCost.tax} €/Monat`} />
                <Spec label="Sprit/Strom" value={`${insights.monthlyCost.fuelOrEnergy} €/Monat`} />
                <Spec label="Wartung" value={`${insights.monthlyCost.maintenance} €/Monat`} />
                <Spec label="Kapitalbindung" value={`${insights.monthlyCost.financing} €/Monat`} />
              </View>
            ) : null}
                ...
                <Text
                  style={[
                    typography.body,
                    {
                      fontWeight: '600',
                      color:
                        insights.priceAssessment.verdict === 'GOOD_DEAL'
                          ? colors.like
                          : insights.priceAssessment.verdict === 'EXPENSIVE'
                            ? colors.warn
                            : insights.priceAssessment.verdict === 'FAIR'
                              ? colors.like
                              : colors.textFaint,
                    },
                  ]}
            ...
            <View style={{ flexDirection: 'row', gap: spacing(2), marginTop: spacing(4) }}>
              ...
              {listing.sourceUrl ? (
                <Pressable
                  style={styles.cta}
                  onPress={() => void Linking.openURL(listing.sourceUrl!)}
                >
